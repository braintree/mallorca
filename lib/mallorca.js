...
function Mallorca(options) {
  ...
  this.upstream = new Upstream(options.upstreamURL, statsdClient.create(options), {
  ...
  this.upstream.setTimeout(this.timeout);
  if (options.allowPerRequestTimeout) {
    this.upstream.allowPerRequestTimeout();
  }
