...
Upstream.prototype.allowPerRequestTimeout = function () {
  this._allowPerRequestTimeout = true;
};
...
UpstreamRequest.prototype.requestTimeoutHeader = function () {
  var value = 0;
  if (typeof this.headers === 'object') {
    Object.keys(this.headers).forEach(function (key) {
      if (key.toLowerCase() == 'x-mallorca-timeout') {
        value = parseInt(this.headers[key], 10) || 0;
      }
    }.bind(this));
  }
  return value;
...
UpstreamRequest.prototype.getTimeout = function () {
  var timeout = 0;
  if (this.parent._allowPerRequestTimeout) {
    timeout = this.requestTimeoutHeader();
  }
  if (timeout == 0 && this.parent.timeout > 0) {
